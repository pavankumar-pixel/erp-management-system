const SUPPLIER_API = "http://localhost:8080/api/suppliers";
...
function saveSupplier() {
    ...
    if (editSupplierId !== null) {
        method = "PUT";
        url = `${SUPPLIER_API}/${editSupplierId}`;
    }
    ...
    .then(() => {
        ...
        clearSupplierForm();
        editSupplierId = null;
        document.getElementById("supplierBtn").innerText = "Save Supplier";
    });
}
...
function editSupplier(id) {
    ...
    .then(suppliers => {
        ...
        editSupplierId = id;
        document.getElementById("supplierBtn").innerText = "Update Supplier";
    });
}

function deleteSupplier(id) {
    if (confirm("Delete Supplier?")) {
        fetch(`${SUPPLIER_API}/${id}`, { method: "DELETE" })
        .then(() => { loadSuppliers(); });
    }
}
